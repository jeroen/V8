...
Unlike what you might be used to from Node or your browser, the global namespace is empty when you start a new context. The global namespace does not have a name either (no `window` object), but you can evaluate `this` in the top scope to access it.
...
Hence to assign or get an object explicitly from the global namespace you could use something like:

```{r}
ct2$eval("var _global = this")
ct2$eval("(function(){var bar = [1,2,3,4]; _global.bar = bar; })()")
ct2$get("bar")
```
